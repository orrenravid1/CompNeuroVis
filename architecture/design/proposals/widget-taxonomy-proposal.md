...
2. **Authored per-widget specs still in core → widget-as-package restructure** (the larger direction). `LevelMarker`, `GridSliceOperatorSpec`, and `MorphologyGeometrySpec` are *authored* (created in `inline/` / `backends/`) **and** *rendered* (`frontends/`). (`GridGeometrySpec` was deleted once surface coordinates became field-owned.) Because a built-in widget is exploded across those sibling trees, its shared authored type is forced into the common ancestor, core. The corrected matrix-safe target is to co-locate each widget's typed declaration objects and frontend implementations in its package, while lowering the declaration into core-owned, kind-keyed, data-only extension specs. Package-owned Python subclass identity must not cross the canonical `AppSpec` boundary: that would work for the current pickle transport but close Unity, Web, WebSocket, and bespoke non-Python rows in the [App Configuration Matrix](../app_configuration_matrix.md). The fix is to **structure built-in widgets as self-contained packages over neutral extension envelopes** — after which core = pure kit (`ExtensionViewSpec` plus equivalent geometry/operator envelopes, `AppSpec` / `Field` / bindings) and the per-widget authored types leave it. Moving them to the frontend *without* the restructure would make the authoring layer import rendering (a layering violation), which is why they stay for now. Large, cross-cutting — its own effort.
