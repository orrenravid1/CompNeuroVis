...
class MorphologyGeometrySpec(GeometrySpec):
    ...
    def __post_init__(self) -> None:
        ...
        if orientations.shape != (n, 3, 3):
            raise ValueError(
                "MorphologyGeometrySpec orientations must have shape (n, 3, 3)"
            )
        if radii.shape != (n,) or lengths.shape != (n,) or xlocs.shape != (n,):
            raise ValueError(
                "MorphologyGeometrySpec radii, lengths, and xlocs must have shape (n,)"
            )
        if len(self.entity_ids) != n or len(self.section_names) != n:
            raise ValueError(
                "MorphologyGeometrySpec entity_ids and section_names must match segment count"
            )
